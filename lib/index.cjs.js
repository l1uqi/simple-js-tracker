...
(function (sessionStoreEnum) {
    sessionStoreEnum["OPSIONS"] = "OPSIONS";
    sessionStoreEnum["PRE_TIMESTAMP"] = "PRE_TIMESTAMP";
    sessionStoreEnum["PAGE_PERFORMANCE"] = "PAGE_PERFORMANCE";
    sessionStoreEnum["PAGE_INFO"] = "PAGE_INFO";
    sessionStoreEnum["UUID"] = "UUID";
...
const initErrorEvent = (options) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (vm != null && vm.config) {
        vm.config.errorHandler = (error) => {
            const pageInfo = getCache(sessionStoreEnum.PAGE_INFO);
            errorCallback({
                errorMsg: error,
                pageInfo: pageInfo,
            });
...
const setVueRouterEvent = (router, options, cb) => {
    ...
    const getStopSecound = (to) => {
        ...
        const pretimestamp = getCache(sessionStoreEnum.PRE_TIMESTAMP) || timestamp;
        setCache(sessionStoreEnum.PRE_TIMESTAMP, timestamp);
        setCache(sessionStoreEnum.PAGE_INFO, JSON.stringify(to.meta));
        let secound = 0;
...
class Exposure {
        ...
        this.observer = new IntersectionObserver((entries, observer) => {
            entries.forEach((item) => {
                if (item.isIntersecting) {
                    ...
                    // 上报
                    autoSendTracker(Object.assign({}, this.params));
                }
            });
        });
    }
    ...
    remove(entry) {
        var _a, _b;
        (_a = this.observer) === null || _a === void 0 ? void 0 : _a.unobserve(entry.el);
...
module.exports = SimpleJsTracker;
